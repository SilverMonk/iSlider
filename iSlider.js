...
function iSlider(opts) {
    ...
    if (this.opts.needPcEvent) {
        if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            this.isPC = false;
        } else {
            this.isPC = true;
            var wrap = typeof this.opts.wrap == 'string' ? this.$(this.opts.wrap) : this.opts.wrap;
            wrap.onselectstart = document.body.ondrag = function() {
                return false;
            }
        }
    }
...
iSlider.prototype = {
    ...
    _touchstart: function(e) {
        ...
        this._touchstartX = self.isPC ? e.pageX : e.touches[0].pageX;
        this._touchstartY = self.isPC ? e.pageY : e.touches[0].pageY;

        this.touchInitPos = this.opts.isVertical ? (self.isPC ? e.pageY : e.touches[0].pageY) : (self.isPC ? e.pageX : e.touches[0].pageX); // 每次move的触点位置
    ...
    _touchmove: function(e) {
        ...
        var self = this;
        if ((e.touches && e.touches.length !== 1) || this.lockSlide) {
            return;
        }

        var gx = Math.abs((self.isPC ? e.pageX : e.touches[0].pageX) - this._touchstartX);
        var gy = Math.abs((self.isPC ? e.pageY : e.touches[0].pageY) - this._touchstartY);
        ...
        if (this.opts.noslide && this.opts.noslide.indexOf(this.index) >= 0) {
            ...
            if (!this.opts.noslideBack || (this.opts.isVertical ? ((self.isPC ? e.pageY : e.touches[0].pageY) - this._touchstartY < 0) : ((self.isPC ? e.pageX : e.touches[0].pageX) - this._touchstartX < 0))) {
                return;
            }
        }


        var currentX = this.opts.isVertical ? (self.isPC ? e.pageY : e.touches[0].pageY) : (self.isPC ? e.pageX : e.touches[0].pageX);
    ...
    prev: function() {
        ...
        if (this.index > 0) {
            ...
            if (this.opts.pageloop && !this._isFirstView) {
                this.index = self.length - 1;
            } else {
                this._itemReset();
                return false;
            }
